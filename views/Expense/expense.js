async function addExpense(event){
    try{
        ...
        const response = await axios.post('http://13.51.199.65:3000/expense/add-expense', obj, {headers:{"Authorization":token}});
    ...
    async function deleteExpense(expenseID){
        try{
            const token = localStorage.getItem('token');
            await axios.delete(`http://13.51.199.65:3000/expense/delete-expense/${expenseID}`,{headers:{"Authorization":token}});
    ...
    document.getElementById('rzp-button1').onclick = async function (e){
        const token = localStorage.getItem('token');
        const response = await axios.get('http://13.51.199.65:3000/purchase/premiummembership',{headers:{"Authorization":token}});
        console.log(response);
        var options = {
            "key": response.data.key_id,
            "order_id":response.data.order.id,
            "handler":async function (response){
                    await axios.post('http://13.51.199.65:3000/purchase/updatetransactionstatus-success',{
    ...
    rzp1.on('payment.failed', async function (response){
        try{
            console.log(response);
            const token = localStorage.getItem('token');
            await axios.post('http://13.51.199.65:3000/purchase/updatetransactionstatus-fail',{
                        order_id:options.order_id,
                        payment_id:response.razorpay_payment_id
                    },{headers:{"Authorization":token}});
    ...
    function showLeaderBoard(){
        document.getElementById('leaderboard').onclick = async () => {
            try{
                const token = localStorage.getItem('token');
                const userLeaderBoardArray = await axios.get('http://13.51.199.65:3000/premium/showLeaderBoard',{headers:{"Authorization":token}});
    ...
    function download(){
        document.getElementById('report').onclick = async () =>{
            try{
                const token = localStorage.getItem('token');
                const response = await axios.get('http://13.51.199.65:3000/user/download',{headers:{"Authorization":token}});
    ...
    function pageFunction2(){
        ...
        document.getElementById('page2').onclick = async ()=>{
            try{
                ...
                const response = await axios.get(`http://13.51.199.65:3000/expense/get-expense/page2?limit=${limit}`,{headers:{"Authorization":token}});
    ...
    window.addEventListener("DOMContentLoaded",async ()=>{
        try{
            ...
            const token = localStorage.getItem('token');
            const limit = localStorage.getItem('limit');
            const response = await axios.get(`http://13.51.199.65:3000/expense/get-expense?limit=${limit}`,{headers:{"Authorization":token}});
            for(var i=0;i<response.data.allExpenses.length;i++){
                showExpenseOnScreen(response.data.allExpenses[i]);
            }
        } catch(error){
    ...
    window.addEventListener("DOMContentLoaded",async ()=>{
        try{
            const token = localStorage.getItem('token');
            const response = await axios.get('http://13.51.199.65:3000/purchase/getstatus',{headers:{"Authorization":token}});
            console.log(response);
    ...
    window.addEventListener("DOMContentLoaded",async ()=>{
        try{
            const token = localStorage.getItem('token');
            const response = await axios.get('http://13.51.199.65:3000/expense/download',{headers:{"Authorization":token}});
    ...
    window.addEventListener("DOMContentLoaded",async ()=>{
        try{
            const token = localStorage.getItem('token');
            const response = await axios.get('http://13.51.199.65:3000/user/total-expense',{headers:{"Authorization":token}});
